/**
 * 견적 산출 로직 (규칙 기반, LLM 미사용)
 * 클라이언트(즉시 미리보기)와 서버 API(검증·저장) 양쪽에서 공용으로 사용한다.
 *
 * 모든 금액 단위는 "만원"(10,000 KRW). 예) 250 = 250만원.
...
// (랜딩 50~200 / 회사소개 150~800 / 반응형 기업 500~1,500 / 쇼핑몰 500~3,000만원대)
export const PROJECT_TYPES: ProjectTypeOption[] = [
  { key: "landing", label: "랜딩페이지", desc: "단일 페이지 · 캠페인/이벤트", base: { min: 50, max: 180 }, weeks: { min: 1, max: 3 } },
  { key: "branding", label: "브랜드/회사소개 홈페이지", desc: "기업 사이트 · 다중 페이지", base: { min: 250, max: 500 }, weeks: { min: 3, max: 6 } },
  { key: "shop", label: "쇼핑몰 · 커머스", desc: "상품/주문/결제", base: { min: 500, max: 1300 }, weeks: { min: 6, max: 12 } },
  { key: "webapp", label: "웹앱 · 플랫폼 (SaaS)", desc: "회원 기반 서비스/플랫폼", base: { min: 700, max: 2000 }, weeks: { min: 8, max: 16 } },
  { key: "mobile", label: "모바일 앱", desc: "iOS/Android 앱", base: { min: 800, max: 2500 }, weeks: { min: 8, max: 18 } },
  { key: "system", label: "업무시스템 · 자동화", desc: "내부 관리/ERP/자동화", base: { min: 600, max: 1800 }, weeks: { min: 6, max: 14 } },
];

export const SCALES: ScaleOption[] = [
  { key: "xs", label: "1 ~ 5 페이지", mult: 1.0, weekMult: 1.0 },
  { key: "s", label: "6 ~ 10 페이지", mult: 1.25, weekMult: 1.25 },
  { key: "m", label: "11 ~ 20 페이지", mult: 1.6, weekMult: 1.5 },
  { key: "l", label: "20 페이지 이상", mult: 2.1, weekMult: 1.9 },
];

export const FEATURES: FeatureOption[] = [
  { key: "auth", label: "회원가입 · 로그인", add: { min: 50, max: 120 }, weeks: 1 },
  { key: "payment", label: "결제 연동", add: { min: 80, max: 200 }, weeks: 1 },
  { key: "booking", label: "예약 · 스케줄", add: { min: 60, max: 150 }, weeks: 1 },
  { key: "community", label: "게시판 · 커뮤니티", add: { min: 50, max: 120 }, weeks: 1 },
  { key: "admin", label: "관리자 페이지", add: { min: 80, max: 200 }, weeks: 1 },
  { key: "cms", label: "콘텐츠 관리(CMS)", add: { min: 60, max: 150 }, weeks: 1 },
  { key: "multilang", label: "다국어 지원", add: { min: 40, max: 100 }, weeks: 1 },
  { key: "map", label: "지도 · 위치", add: { min: 20, max: 60 }, weeks: 1 },
  { key: "chat", label: "실시간 채팅", add: { min: 80, max: 180 }, weeks: 1 },
  { key: "ai", label: "AI 기능 연동", add: { min: 100, max: 300 }, weeks: 2 },
  { key: "api", label: "외부 API 연동", add: { min: 50, max: 150 }, weeks: 1 },
  { key: "analytics", label: "통계 · 대시보드", add: { min: 60, max: 160 }, weeks: 1 },
];

// 시장가 기준 디자인 수준별 폭(템플릿 ↔ 풀커스텀 약 2배)을 반영해 배수 확대.
export const DESIGNS: DesignOption[] = [
  { key: "template", label: "템플릿 기반", desc: "검증된 레이아웃 · 합리적 비용", mult: 0.75 },
  { key: "custom", label: "세미 커스텀", desc: "메인 맞춤 + 서브 템플릿", mult: 1.0 },
  { key: "premium", label: "풀 커스텀 · 브랜딩 포함", desc: "전체 맞춤 디자인 · 모션", mult: 1.5 },
];

export const TIMELINES: TimelineOption[] = [
  { key: "urgent", label: "급함 (1개월 내)", mult: 1.25, weekMult: 0.7 },
  { key: "normal", label: "보통 (2 ~ 3개월)", mult: 1.0, weekMult: 1.0 },
  { key: "flexible", label: "여유 (협의)", mult: 0.95, weekMult: 1.1 },
];
...
export interface EstimateResult {
  min: number; // 만원
  max: number; // 만원
  rangeLabel: string; // "350만 ~ 600만원"
  weeksLabel: string; // "예상 4 ~ 8주"
  breakdown: EstimateBreakdownItem[];
  maintenanceLabel: string; // 월 유지보수 안내
  note: string;
}
...
export function computeEstimate(input: EstimateInput): EstimateResult {
  ...
  const breakdown: EstimateBreakdownItem[] = [
    { label: `${type.label} (기본 · ${scale.label})`, amount: `${formatManwon(round10(type.base.min * scale.mult))} ~ ${formatManwon(round10(type.base.max * scale.mult))}원` },
