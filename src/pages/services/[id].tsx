...
// SSG 전환 — router.query 기반 CSR이라 서버 HTML에 메타가 없던 문제 해소 (데이터는 로컬 배열)
export default function ServiceDetail({ id }: ServiceDetailProps) {
    ...
    const breadcrumbJsonLd = {
        ...
        itemListElement: [
            ...
            { "@type": "ListItem", position: 2, name: "서비스", item: "https://nqsolution.kr/services" },
            { "@type": "ListItem", position: 3, name: service.title, item: `https://nqsolution.kr/services/${service.id}` },
        ],
    };
    ...
    return (
        <>
        <Seo
            title={`${service.title} | 서비스 - NQ Solution (엔큐솔루션)`}
...
export const getStaticPaths: GetStaticPaths = async () => ({
    // 데이터가 로컬 배열이라 4개 slug 전부 빌드 타임 생성 — DB 불필요
    paths: servicesData.map((s) => ({ params: { id: s.id } })),
    fallback: false,
});

export const getStaticProps: GetStaticProps<ServiceDetailProps> = async ({ params }) => {
    const id = String(params?.id ?? "");
    if (!servicesData.some((s) => s.id === id)) return { notFound: true };
    return { props: { id } };
};
